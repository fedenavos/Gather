  ...
  const submitData = async (e) => {
    ...
    try {
      ...
      if (response.status === 200) {
        props.handleClose();
        props.refresh();
        props.handleNotification("success", "", t("trip-ended"));
      }
      if (response.status === 400) {
        const data = await response.json();
        console.log(data);
        props.handleNotification("error", "", t("couldnt-end-trip"));
      }
  ...
  return (
    <Modal show={props.show} onHide={props.handleClose} size="md">
      <Modal.Header closeButton>
        <Modal.Title>{t("end-trip")}</Modal.Title>
      </Modal.Header>
      <Modal.Body>
        <Container>
          <Row>
            <p>{t("end-trip-confirmation")}</p>
            <p className="strong">{t("end-trip-description")}</p>
          </Row>
        </Container>
      </Modal.Body>
      <Modal.Footer>
